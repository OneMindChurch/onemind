...
const HisFileYoutube = (props: any) => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      let res = await axios.get(
        "https://www.hanmaumchurch.net/api/hisfile/list"
      );
      setPosts(res.data);
    };
    fetchPosts();
            ...
            <tbody>
              {currentPosts.map((post) => (
                <tr key={post.id}>
                  <Td>{post.id}</Td>
                  <Td>
                    <StyledLink to={"/showhisfile/" + String(post.id)}>
                      {post.title}
                    </StyledLink>
                  </Td>
